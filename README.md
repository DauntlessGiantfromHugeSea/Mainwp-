...
- **Einrichtung auf Knopfdruck**: einmal das Passwort des Speichers eintippen, und das Panel erzeugt den Schlüssel, holt den Wirtsschlüssel, legt den eigenen Schlüssel per `ssh-copy-id` auf dem Speicher ab und legt das Repository an — Schritt für Schritt mit Ergebnis. Das Passwort wird nur für diesen einen Vorgang benutzt und weder in der Datenbank noch auf der Platte abgelegt; es erreicht ssh über ein Hilfsprogramm, das es aus der Prozessumgebung liest, damit es nicht in der Prozessliste steht
- Schlüssel, `known_hosts` und die ssh-Konfiguration liegen unter `storage/restic/` — unter demselben Benutzer, unter dem auch der Webserver läuft, statt in einem `/root/.ssh`, das er nicht lesen kann. Die Konfiguration wird restic per `-o sftp.command=ssh -F …` mitgegeben: ssh nimmt sein Heimatverzeichnis aus der Passwortdatenbank des Systems, nicht aus `HOME`, und läse sie sonst nie
- Der **Wirtsschlüssel** lässt sich auch von Hand in zwei Schritten übernehmen: erst abrufen und den Fingerabdruck anzeigen, dann nach dem Vergleich bestätigen. `StrictHostKeyChecking` bleibt an. Meldet der Speicher später einen anderen Schlüssel als den hinterlegten, bricht das Panel ab, statt ihn stillschweigend zu ersetzen
